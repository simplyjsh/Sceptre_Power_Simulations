...
normalize_deseq <- function(sce, expr_quantile = c(0, 0.9), norm_genes = NULL,
  ...
  size_factors <- DESeq2::estimateSizeFactorsForMatrix(counts,
    locfunc = locfunc,
...
test_differential_expression <- function(sce, pert_level, max_dist = NULL,
  ...
  output <- bplapply(perts,
    FUN = test_de, sce = sce, pert_level = pert_level,
...
de_DEsingle <- function(pert_object, assay = "normcounts", ...) {
  ...
  # reformat output
  output <- data.frame(
    gene = rownames(de_results), de_results, stringsAsFactors = FALSE,
    row.names = NULL
  )
...
de_LFC <- function(pert_object, assay = "logcounts", pseudocount = 1, ...) {
  ...
  # reformat output
  output <- data.frame(
    gene = rownames(avg_genex), avg_genex, stringsAsFactors = FALSE,
    row.names = NULL
  )
...
test_de <- function(pert, sce, pert_level, cell_batches, pert_input_function, max_dist, de_function,
  ...
  pert_object <- pert_input_function(pert,
    sce = sce, pert_level = pert_level,
  ...
  if (!is.null(max_dist)) {
    ...
    pert_object <- filt_max_dist_pert(pert_object,
      pert_level = pert_level, pert = pert,
      max_dist = max_dist
    )
  }
...
# generate input for one perturbation without sampling control cells
# (all non-perturbed cells are included as controls)
...
annotate_cells_and_expr <- function(de_output, sce, pert_level) {
  ...
  cells_per_pert <- data.frame(
    perturbation = names(cells_per_pert), cells = cells_per_pert,
  ...
  avg_expr <- data.frame(
    gene = names(avg_expr), avg_expr = avg_expr, stringsAsFactors = FALSE,
    row.names = NULL
  )
...
annotate_dist_to_gene <- function(de_output, sce, pert_level) {
  ...
  gene_coords <- data.frame(
    gene_chr = as.character(seqnames(gene_coords)),
