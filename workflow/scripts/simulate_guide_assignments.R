...
# [Note] The 6000 is to make sure there are enough n_ctrl cells but this
#        calculation is arbitrary otherwise
num_cells <- num_cells_per_pert[length(num_cells_per_pert)] * 5 + 6000
...
rowData(altExp(sce, "grna_perts"))$target_name <- sub(
  "_.*",
  "",
  rownames(altExp(sce, "grna_perts"))
)
